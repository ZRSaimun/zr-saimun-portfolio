...
export function attachWeather(onChange){
 ...
 async function fetchLive(){const d=city(),id=++sequence;live.disabled=true;status.textContent=`Loading ${d.city} weather…`;try{
  ...
  if(id!==sequence)return;currentRecord=record;liveMode=true;window.zrLiveWeather=record;window.dispatchEvent(new CustomEvent('zr:live-weather',{detail:record}));apply(weatherCode(record.data.current.weather_code,record.data.current.is_day));caption();
 }catch{if(id!==sequence)return;liveMode=false;currentRecord=null;window.zrLiveWeather=null;apply(select.value==='auto'?d.weather:select.value);status.textContent=`${d.city}: live weather unavailable. Cinematic fallback — not live conditions.`;}finally{if(id===sequence)live.disabled=false;}}
