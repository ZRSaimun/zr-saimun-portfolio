...
export function startPremiumWorld(){
 ...
 const globe=add(new THREE.SphereGeometry(10,48,32),new THREE.MeshStandardMaterial({map:earthTexture,roughness:.35,metalness:.2}),0,15,0);
 ...
 gates.push({d,group,frame,glass,photo,border,broken:false,angle:a});});
 ...
 function enter(){if(nearest>=0){speed=0;window.zrOpenCity?.(gates[nearest].d.city);}}
 ...
 function frame(now){requestAnimationFrame(frame);const dt=Math.min(.035,(now-last)/1000);last=now;const on=visible(),stop=blocked()||window.zrMotionPaused||!on;
  ...
  if(!stop){time+=dt;globe.rotation.y+=dt*.045;orbit.rotation.z+=dt*.06;aircraft.position.set(Math.sin(time*.2)*15,16+Math.sin(time*.15)*4,Math.cos(time*.2)*15);aircraft.rotation.set(.1,time*.2+Math.PI/2,-.18);
   ...
   let near=-1,min=15;gates.forEach((g,i)=>{const d=rig.car.position.distanceTo(g.group.position);if(d<min){min=d;near=i;}if(mode==='drive'&&!g.broken&&!g.d.sacred&&d<2.7&&Math.abs(speed)>2){impact(Math.abs(speed));shatter(g);speed*=.45;}});
