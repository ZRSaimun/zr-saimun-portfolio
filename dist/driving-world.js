...
export function startPremiumWorld(){
 ...
 window.zrDrive={travel,travelCity:city=>{const index=destinations.findIndex(d=>d.city===city);if(index>=0)travel(index);},setMode,repair,get state(){return {speed,damage,weather,city:activeCity.city,mode,broken:gates.filter(g=>g.broken).length,position:{x:rig.car.position.x,z:rig.car.position.z}};}};
 studioWeather();
